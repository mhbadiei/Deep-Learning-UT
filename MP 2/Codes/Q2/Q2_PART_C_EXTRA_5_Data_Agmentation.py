...
class noteRecognizer:
    ...
    def loadData(self, path):
        ...
        for fileName in self.dataset['Train']:
            ...
            #Data Augmentation    
            D2=xTrain_
            L2 = fileName.split('.wav')[0].split('_')[1]
            if fileName == self.dataset['Train'][0]:
                D1 = xTrain_ 
                L1 = fileName.split('.wav')[0].split('_')[1]
            else:
                if L1 == L2:
                    self.dataAugmentation(D1,D2,L1)
                D1 = xTrain_ 
                L1 = fileName.split('.wav')[0].split('_')[1]
    ...
    def dataAugmentation(self, d1, d2, label):
        d_ = np.add(d1, d2)/2
        self.xTrain = np.append(self.xTrain, d_ , axis=0)
        self.yTrain.extend(np.repeat(label, np.shape(d_)[0])) 
...
if __name__ == "__main__" :
    ...
    # Time frame should be in unit of mili second     
    model = noteRecognizer(compressionLength = 400, timeFrame=300, notesLength = 20, instrumentLength = 20)
    ...
    model.train(epochs=50, batch_size=256, validation_split=0.15, learningRate=0.01)
    model.plotAccuracy()
    model.plotLoss()
    model.showPlot()
    model.testModel()